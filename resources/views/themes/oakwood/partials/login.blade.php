<div class="ui xsmall modal" id="loginModal">
  	<div class="content">
	    <form method="POST" action="{{route('auth.login.store')}}" id="loginForm" class="ui form">
			{{csrf_field()}}
			<div class="field">
				<label>Email</label>
			...
			<button type="submit" class="ui primary button pull-right">Log In</button>
			<a href="{{ route('auth.recover') }}">Forgot password?</a> <br />
			<a href="{{ route('auth.register.form') }}">Not yet registered?</a>
		</form>
		<br />
