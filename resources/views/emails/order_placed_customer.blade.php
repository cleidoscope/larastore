                                ...
                                <tr>
                                    <td style="{{ $fontFamily }} {{ $style['email-body_cell'] }}">
                                        ...
                                        <p style="{{ $style['paragraph'] }}">
                                            You have placed an order in <strong>{{ $order->store->name }}</strong>. The details are below:
                                        </p>
                                        <br />
                                        <table style="width: 100%; border: solid 1px #eee;" cellspacing="0" cellpadding="0">
                                            <thead style="background-color: rgba(21,93,140,.05);">
                                            ...
                                            <tr style="vertical-align: top;">
                                                ...
                                                <td style="padding: 10px">
                                                    @if( $item->discounted_price ) 
                                                    <strong style="font-size: 14px">{{ Helpers::currencyFormat($item->discounted_price) }}</strong> <br />
                                                    <del>{{ Helpers::currencyFormat($item->price) }}</del>
                                                    {!! Helpers::getDiscountHTML($item->price, $item->discounted_price) !!} 
                                                    @else
                                                    <strong style="font-size: 14px">{{ Helpers::currencyFormat($item->price) }}</strong>
                                                    @endif
                                                </td>
                                        ...
                                        <br />
                                        <p style="{{ $style['paragraph'] }}text-align: right;">
                                            <small>Subtotal: {{ Helpers::currencyFormat($total) }}</small> <br />
                                            <small>Shipping fee: {{ Helpers::currencyFormat($order->shipping_fee) }}</small> <br />
                                            @if( $order->voucher )
                                            <small>- {{ Helpers::currencyFormat($order->voucher->discount) }}</small><br />
                                            @endif
                                            <h3 style="text-align: right;">Total: {{ Helpers::currencyFormat($order->total) }}</h3>
                                        </p>
                                        <!-- Action Button -->
                                        @if (isset($actionUrl))
